...
class Router
{
    ...
    public function dispatch(Request $request): Response
    {
        ...
        if (!$route) {
            error_log("Route not found for $method $path");
        ...
        if (is_array($handler)) {
            ...
            if (is_string($controller)) {
                try {
                    ...
                    error_log("Router::runHandler - Error creating controller: " . $e->getMessage());
                    throw $e;
                }
            }

            try {
                $result = app()->getContainer()->call([$controller, $method], [
                    'request' => $request,
                ]);
                return $result;
            } catch (\Exception $e) {
                error_log("Router::runHandler - Error calling method: " . $e->getMessage());
                throw $e;
            }
        }
